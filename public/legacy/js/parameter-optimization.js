...
class ParameterOptimizer {
  ...
  async startDataCollection() {
    ...
    try {
      ...
      // Hide the bottom-right panel during data collection — flash overlay is the only guide
      if (this.ui) {
  ...
  checkSystemReady() {
    ...
    if (warnings.length > 0) {
      console.log('System warnings (suppressed from UI):', warnings);
    }
    ...
    return new Promise((resolve) => {
      ...
      overlay.innerHTML = `
        <div style="
          ...
          border-radius: 16px;
        ">
          <h1 style="color: #64c8ff; font-size: 36px; margin: 0 0 20px 0;">Pareto Optimization</h1>
          <p style="font-size: 20px; color: #ccc; margin-bottom: 30px;">
            You will visit <strong>9 positions</strong> on the screen. At each position:
          </p>
          <div style="text-align: left; font-size: 22px; line-height: 2; margin: 0 auto; max-width: 550px;">
            <div style="color: #ff6b6b; margin-bottom: 8px;">
              <strong>1.</strong> A <span style="color: #ff4444;">RED target</span> appears — <strong>move your head towards it</strong>
            </div>
            <div style="color: #ffdd57; margin-bottom: 8px;">
              <strong>2.</strong> When steady, <strong>press SPACE</strong>
            </div>
            <div style="color: #ffaa00; margin-bottom: 8px;">
              <strong>3.</strong> <strong>Hold completely still</strong> for 2.5 seconds
            </div>
            <div style="color: #44ff44; margin-bottom: 8px;">
              <strong>4.</strong> A <span style="color: #44ff44;">GREEN target</span> appears — <strong>move to it</strong>
            </div>
            <div style="color: #aaa; margin-bottom: 8px;">
              <strong>5.</strong> Repeat for all 9 positions
            </div>
          </div>
          <p style="font-size: 18px; color: #888; margin: 25px 0 10px;">
            Messages near the target will guide you at every step.
          </p>
          <div style="
  ...
  setupUI() {
    ...
    if (!document.getElementById('pareto-flash-styles')) {
      ...
      flashStyle.textContent = `
        @keyframes paretoFlashPulse { from { transform: scale(1); opacity: 0.85; } to { transform: scale(1.06); opacity: 1; } }
  ...
  async collectAtAllPositions() {
    for (let i = 0; i < this.targetPositions.length; i++) {
      ...
      // Pause between positions so "GOOD!" is visible, then clear before next target
    ...
    this.flashOverlay.innerHTML = `
      <div style="text-align: center; padding: 12px 24px; background: rgba(0,0,0,0.8); border-radius: 12px; display: inline-block;">
        <div style="
          ...
          animation: paretoFlashPulse 0.4s ease-in-out infinite alternate;
          letter-spacing: 2px;
        ">${text}</div>
  ...
  async collectAtPosition(position, nextPosition = null) {
    ...
    // Flash overlay is the ONLY instruction — no bottom-right panel
    const posNum = this.currentPosition + 1;
    const totalPos = this.targetPositions.length;
    this.showFlashMessage(
      'MOVE TO RED TARGET',
      '#ff4444', 0,
      `<span style="color:#ffdd57; font-size:24px; font-weight:bold;">PRESS SPACE when ready</span> &nbsp;·&nbsp; Position ${posNum} of ${totalPos}`
    );
      ...
      const handleKeyPress = (event) => {
        if (event.code === 'Space' && !isCollecting && this.isCollecting !== false) {
          ...
          this.showFlashMessage(
            'HOLD STILL!',
            '#ffaa00', 0,
            `Recording for ${T_I_WAIT/1000} seconds... don't move your head`
          );
          ...
          if (!isLastPosition) {
            setTimeout(() => {
              ...
              this.showFlashMessage(
                'MOVE TO GREEN!',
                '#44ff44', 0,
                'Move your head towards the green circle',
          ...
          setTimeout(() => {
            ...
            if (isLastPosition) {
              this.showFlashMessage('ALL DONE!', '#00ff66', 2500, 'Starting parameter optimization...',
                pixelX, pixelY);
            } else {
              this.showFlashMessage('GOOD!', '#00ff66', 0, 'Next position coming...',
                nextPixelX, nextPixelY);
            }
  ...
  finishDataCollection() {
    ...
    this.status.textContent = 'Roberto Data Collection Complete!';
    this.instructions.innerHTML = `
      <span style="color: #00ff00;">✓ ${validPositions.length}/${this.targetPositions.length} positions with click timing</span><br>
      Starting parameter optimization...<br>
      <span style="color: #44aaff;">✓ Testing One Euro Filter AND Exponential Smoothing</span>
    `;
  ...
  calculateVarianceRobertoMethod(timingData, filteredData) {
    ...
    if (this.rawVarianceCount % 1000 === 0) {
      ...
      if (avgRawVariance > 50) {
        console.warn(`High jitter detected (${avgRawVariance.toFixed(0)}px) - suppressed from UI`);
      }
    }
  ...
  displayResults(paretoFront, paretoFrontExponential) {
    ...
    this.status.textContent = 'Optimization Complete!';

    // Store for zip export
    this._lastParetoFront = paretoFront;
    this._lastParetoFrontExponential = paretoFrontExponential;

    // Show a big "Download All" prompt — user click satisfies Chrome's gesture requirement
    this._showDownloadPrompt(paretoFront, paretoFrontExponential);
    
    // Create button container at the top for easy access
    ...
    zipBtn.style.cssText = `
      ...
      font-size: 15px;
    `;
    zipBtn.onclick = () => this.downloadAllAsZip(paretoFront, paretoFrontExponential);

    // Upload & re-analyze button
    const uploadBtn = document.createElement('button');
    uploadBtn.textContent = '📂 Upload & Re-analyze';
    uploadBtn.style.cssText = `
      ...
      font-size: 15px;
    `;
    uploadBtn.onclick = () => this.loadRawTrialData();
    ...
    // Insert buttons at the top (after status and progress)
    if (this.ui.children.length >= 2) {
      this.ui.insertBefore(buttonContainer, this.instructions);
    } else {
      this.ui.appendChild(buttonContainer);
    }
    
    // Hide the verbose instructions panel — all data is in the ZIP
  ...
  _showDownloadPrompt(paretoFront, paretoFrontExponential) {
    const overlay = document.createElement('div');
    overlay.style.cssText = `
      position: fixed; top: 0; left: 0; width: 100%; height: 100%;
    ...
    overlay.innerHTML = `
      <div style="text-align: center; padding: 50px 60px; background: rgba(30,30,40,0.98);
        border: 2px solid #22cc66; border-radius: 16px; max-width: 500px;">
        <div style="font-size: 48px; margin-bottom: 16px;">✅</div>
        <h1 style="color: #22cc66; font-size: 32px; margin: 0 0 12px;">Optimization Complete!</h1>
        <p style="color: #ccc; font-size: 18px; margin: 0 0 30px;">
          All results are ready. Click below to download everything as a single ZIP file.
        </p>
        <button id="pareto-download-zip-btn" style="
          padding: 18px 50px; font-size: 22px; font-weight: bold;
          background: linear-gradient(135deg, #22cc66, #118844); color: white;
          border: none; border-radius: 10px; cursor: pointer;
          box-shadow: 0 0 20px rgba(34,204,102,0.4);
        ">📦 Download All as ZIP</button>
        <div style="margin-top: 20px;">
          <span id="pareto-skip-download" style="color: #888; font-size: 14px; cursor: pointer;
            text-decoration: underline;">Skip — I'll download individually</span>
        </div>
      </div>
    `;
    document.body.appendChild(overlay);
    ...
    // Pre-build the zip blob NOW so the download link click is synchronous
    this._buildZipBlob(paretoFront, paretoFrontExponential).then(blob => {
      this._pendingZipBlob = blob;
    });

    document.getElementById('pareto-download-zip-btn').onclick = async function () {
      this.disabled = true;
      this.textContent = '⏳ Preparing ZIP...';
      try {
        const blob = self._pendingZipBlob || await self._buildZipBlob(paretoFront, paretoFrontExponential);
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const url = URL.createObjectURL(blob);
        ...
        URL.revokeObjectURL(url);
        console.log(`✅ Downloaded ZIP (${(blob.size / 1024).toFixed(1)} KB)`);
        dismiss();
      } catch (err) {
        console.error('ZIP download failed:', err);
        this.textContent = '❌ Failed — use individual buttons';
        setTimeout(dismiss, 2000);
      }
    };

    document.getElementById('pareto-skip-download').onclick = dismiss;

    const spaceHandler = (e) => {
      if (e.code === 'Space') {
        e.preventDefault();
        document.removeEventListener('keydown', spaceHandler);
        document.getElementById('pareto-download-zip-btn')?.click();
      }
    };
    document.addEventListener('keydown', spaceHandler);
  }
